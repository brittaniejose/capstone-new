...
function Comments({ data, resource }) {
  ...
  const getComments = async () => {
    const response = await fetch(
      `http://localhost:3000/comments/${resource}/${data.id}`,
      {
        method: "GET",
  ...
  return (
    <React.Fragment>
      {commentsFetched ? (
        ...
        <Divider sx={{ mb: 0 }} />
        <Typography sx={{ mt: 5 }}>Be the first to comment!</Typography>
